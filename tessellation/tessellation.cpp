...
public:
	virtual int32_t			init(CVulkanFramework* pFramework)
	{
		...
		m_pFramework->rotation = glm::vec3(-380.0f, 18.5f, 0.0f);
		m_pFramework->title = "Vulkan Example - Tessellation shader (PN Triangles)";
	...
	void loadMeshes()
	{
		m_pFramework->loadMesh(m_pFramework->getAssetPath() + "models/lowpoly/bearmug.dae", &meshes.object, vertexLayout, 4.0f);
	}

	void loadTextures()
	{
		m_pFramework->textureLoader->loadTexture(
			m_pFramework->getAssetPath() + "textures/bearmug.ktx",
	...
	void preparePipelines()
	{
		...
		shaderStages[0] = m_pFramework->loadShader(m_pFramework->getAssetPath() + "shaders/tessellation/base.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = m_pFramework->loadShader(m_pFramework->getAssetPath() + "shaders/tessellation/base.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
		shaderStages[2] = m_pFramework->loadShader(m_pFramework->getAssetPath() + "shaders/tessellation/pntriangles.tesc.spv", VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT);
		shaderStages[3] = m_pFramework->loadShader(m_pFramework->getAssetPath() + "shaders/tessellation/pntriangles.tese.spv", VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT);
		...
		shaderStages[2] = m_pFramework->loadShader(m_pFramework->getAssetPath() + "shaders/tessellation/passthrough.tesc.spv", VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT);
		shaderStages[3] = m_pFramework->loadShader(m_pFramework->getAssetPath() + "shaders/tessellation/passthrough.tese.spv", VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT);
