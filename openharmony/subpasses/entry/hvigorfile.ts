  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/subpasses'), path.join(rawfile, 'shaders/glsl/subpasses'), /\.spv$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'samplebuilding.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'samplebuilding_glass.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'colored_glass_rgba.ktx');
      }
    }
  ]
}
