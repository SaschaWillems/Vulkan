  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/texture'), path.join(rawfile, 'shaders/glsl/texture'), /\.spv$/);
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'metalplate01_rgba.ktx');
      }
    }
  ]
}
