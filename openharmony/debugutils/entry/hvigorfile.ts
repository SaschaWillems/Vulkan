  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/debugutils'), path.join(rawfile, 'shaders/glsl/debugutils'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'treasure_glow.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'treasure_smooth.gltf');
      }
    }
  ]
}
