  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/tessellation'), path.join(rawfile, 'shaders/glsl/tessellation'), /\.spv$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'deer.gltf');
      }
    }
  ]
}
