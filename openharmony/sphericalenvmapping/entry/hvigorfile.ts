  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/sphericalenvmapping'), path.join(rawfile, 'shaders/glsl/sphericalenvmapping'), /\.spv$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'chinesedragon.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'matcap_array_rgba.ktx');
      }
    }
  ]
}
