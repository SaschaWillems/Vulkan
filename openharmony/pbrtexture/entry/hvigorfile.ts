  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/pbrtexture'), path.join(rawfile, 'shaders/glsl/pbrtexture'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models/cerberus'), path.join(rawfile, 'models/cerberus'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'cube.gltf');
        copyFiles(path.join(assetPath,  'textures/hdr'), path.join(rawfile, 'textures/hdr'), 'gcanyon_cube.ktx');
      }
    }
  ]
}
