  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/computecloth'), path.join(rawfile, 'shaders/glsl/computecloth'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'sphere.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'vulkan_cloth_rgba.ktx');
      }
    }
  ]
}
