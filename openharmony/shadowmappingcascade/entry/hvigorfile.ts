  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/shadowmappingcascade'), path.join(rawfile, 'shaders/glsl/shadowmappingcascade'), /\.spv$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'oaktree.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'terrain_gridlines.gltf');
      }
    }
  ]
}
