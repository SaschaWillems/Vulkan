  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/particlesystem'), path.join(rawfile, 'shaders/glsl/particlesystem'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'fireplace.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'fireplace_colormap_rgba.ktx');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'fireplace_normalmap_rgba.ktx');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'particle_fire.ktx');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'particle_smoke.ktx');
      }
    }
  ]
}
