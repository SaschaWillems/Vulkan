  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/computecullandlod'), path.join(rawfile, 'shaders/glsl/computecullandlod'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'suzanne_lods.gltf');
      }
    }
  ]
}
