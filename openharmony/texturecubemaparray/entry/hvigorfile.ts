  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/texturecubemaparray'), path.join(rawfile, 'shaders/glsl/texturecubemaparray'), /\.spv$/);
        ...
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'teapot.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'torusknot.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'venus.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'cubemap_array.ktx');
      }
    }
  ]
}
