  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/gltfloading'), path.join(rawfile, 'shaders/glsl/gltfloading'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models/FlightHelmet/glTF'), path.join(rawfile, 'models/FlightHelmet/glTF'), /^.+\..+$/);
      }
    }
  ]
}
