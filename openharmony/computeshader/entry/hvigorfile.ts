  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/computeshader'), path.join(rawfile, 'shaders/glsl/computeshader'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'textures'),           path.join(rawfile, 'textures'),                   'vulkan_11_rgba.ktx');
      }
    }
  ]
}
