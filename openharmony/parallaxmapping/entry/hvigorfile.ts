  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/parallaxmapping'), path.join(rawfile, 'shaders/glsl/parallaxmapping'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'plane.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'rocks_color_rgba.ktx');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'rocks_normal_height_rgba.ktx');
      }
    }
  ]
}
