  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/indirectdraw'), path.join(rawfile, 'shaders/glsl/indirectdraw'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'plane_circle.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'plants.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'sphere.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'ground_dry_rgba.ktx');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'texturearray_plants_rgba.ktx');
      }
    }
  ]
}
