  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/vulkanscene'), path.join(rawfile, 'shaders/glsl/vulkanscene'), /\.spv$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'cube.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'vulkanscenebackground.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'vulkanscenelogos.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'vulkanscenemodels.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'cubemap_vulkan.ktx');
      }
    }
  ]
}
