  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/shadowmappingomni'), path.join(rawfile, 'shaders/glsl/shadowmappingomni'), /\.spv$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'cube.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'shadowscene_fire.gltf');
      }
    }
  ]
}
