  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/gltfskinning'), path.join(rawfile, 'shaders/glsl/gltfskinning'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models/CesiumMan/glTF'), path.join(rawfile, 'models/CesiumMan/glTF'), /^.+\..+$/);
      }
    }
  ]
}
