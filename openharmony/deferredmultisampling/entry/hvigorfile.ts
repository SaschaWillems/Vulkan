  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/deferredmultisampling'), path.join(rawfile, 'shaders/glsl/deferredmultisampling'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models/armor'), path.join(rawfile, 'models/armor'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'deferred_box.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'stonefloor02_normal_rgba.ktx');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'stonefloor02_color_rgba.ktx');
      }
    }
  ]
}
