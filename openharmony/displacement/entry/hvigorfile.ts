  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/displacement'), path.join(rawfile, 'shaders/glsl/displacement'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'displacement_plane.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'stonefloor03_color_height_rgba.ktx');
      }
    }
  ]
}
