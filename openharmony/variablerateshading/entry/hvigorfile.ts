  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/variablerateshading'), path.join(rawfile, 'shaders/glsl/variablerateshading'), /\.spv$/);
        copyFiles(path.join(assetPath,  'models/sponza'), path.join(rawfile, 'models/sponza'),  /^.+\..+$/);
      }
    }
  ]
}
