  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/hdr'), path.join(rawfile, 'shaders/glsl/hdr'), /^.+\..+$/);
        ...
        copyFiles(path.join(assetPath,  'textures/hdr'), path.join(rawfile, 'textures/hdr'), 'uffizi_cube.ktx');
      }
    }
  ]
}
