  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/instancing'), path.join(rawfile, 'shaders/glsl/instancing'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'lavaplanet.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'rock01.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'lavaplanet_rgba.ktx');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'texturearray_rocks_rgba.ktx');
      }
    }
  ]
}
