  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/texturearray'), path.join(rawfile, 'shaders/glsl/texturearray'), /\.spv$/);
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'texturearray_rgba.ktx');
      }
    }
  ]
}
