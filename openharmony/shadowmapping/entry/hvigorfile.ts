  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/shadowmapping'), path.join(rawfile, 'shaders/glsl/shadowmapping'), /\.spv$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'samplescene.gltf');
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'vulkanscene_shadow.gltf');
      }
    }
  ]
}
