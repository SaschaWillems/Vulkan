  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/multithreading'), path.join(rawfile, 'shaders/glsl/multithreading'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'retroufo_red_lowpoly.gltf');
