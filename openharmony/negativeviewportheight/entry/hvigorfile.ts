  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/negativeviewportheight'), path.join(rawfile, 'shaders/glsl/negativeviewportheight'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'texture_orientation_ccw_rgba.ktx');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'texture_orientation_cw_rgba.ktx');
      }
    }
  ]
}
