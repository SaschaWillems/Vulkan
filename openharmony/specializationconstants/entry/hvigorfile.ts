  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/specializationconstants'), path.join(rawfile, 'shaders/glsl/specializationconstants'), /\.spv$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'color_teapot_spheres.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'metalplate_nomips_rgba.ktx');
      }
    }
  ]
}
