  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/multiview'), path.join(rawfile, 'shaders/glsl/multiview'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'sampleroom.gltf');
      }
    }
  ]
}
