  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/computeparticles'), path.join(rawfile, 'shaders/glsl/computeparticles'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'particle_gradient_rgba.ktx');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'particle01_rgba.ktx');
      }
    }
  ]
}
