  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/multisampling'), path.join(rawfile, 'shaders/glsl/multisampling'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'voyager.gltf');
      }
    }
  ]
}
