  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/deferred'), path.join(rawfile, 'shaders/glsl/deferred'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'stonefloor01_color_rgba.ktx');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'stonefloor01_normal_rgba.ktx');
        copyFiles(path.join(assetPath, 'models'), path.join(rawfile, 'models'), 'deferred_floor.gltf');
