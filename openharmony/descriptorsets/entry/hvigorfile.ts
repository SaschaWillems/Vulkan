  ...
  plugins: [
    {
      ...
      apply(node: any) {
        ...
        copyFiles(path.join(shaderPath, 'glsl/descriptorsets'), path.join(rawfile, 'shaders/glsl/descriptorsets'), /^.+\..+$/);
        copyFiles(path.join(assetPath,  'models'), path.join(rawfile, 'models'), 'cube.gltf');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'crate01_color_height_rgba.ktx');
        copyFiles(path.join(assetPath,  'textures'), path.join(rawfile, 'textures'), 'crate02_color_height_rgba.ktx');
      }
    }
  ]
}
