...
void vkglTF::Texture::fromglTfImage(tinygltf::Image &gltfimage, vks::VulkanDevice *device, VkQueue copyQueue)
{
	this->device = device;
...
void vkglTF::Model::drawNode(Node *node, VkCommandBuffer commandBuffer)
{
	if (node->mesh) {
		for (Primitive *primitive : node->mesh->primitives) {
