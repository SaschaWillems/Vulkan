...
namespace
{
	...
	bool tryConvertAndroidGamepadKey(const uint32_t rawKeyCode, vks::VirtualKey& rVirtualKey, uint32_t& rOldStyleKey)
	{
		using namespace vks;
		...
		case AKEYCODE_BUTTON_A:
			rVirtualKey = VirtualKey::GamePadButtonA;
			rOldStyleKey = GAMEPAD_BUTTON_A;
			return true;
		case AKEYCODE_BUTTON_B:
			rVirtualKey = VirtualKey::GamePadButtonB;
			rOldStyleKey = GAMEPAD_BUTTON_B;
			return true;
		case AKEYCODE_BUTTON_X:
			rVirtualKey = VirtualKey::GamePadButtonX;
			rOldStyleKey = GAMEPAD_BUTTON_X;
			return true;
		case AKEYCODE_BUTTON_Y:
			rVirtualKey = VirtualKey::GamePadButtonY;
			rOldStyleKey = GAMEPAD_BUTTON_Y;
			return true;
		case AKEYCODE_BUTTON_L1:
			rVirtualKey = VirtualKey::GamePadButtonLeftShoulder1;
			rOldStyleKey = GAMEPAD_BUTTON_L1;
			return true;
		case AKEYCODE_BUTTON_R1:
			rVirtualKey = VirtualKey::GamePadButtonRightShoulder1;
			rOldStyleKey = GAMEPAD_BUTTON_R1;
			return true;
		case AKEYCODE_BUTTON_START:
			rVirtualKey = VirtualKey::GamePadButtonStart;
			rOldStyleKey = GAMEPAD_BUTTON_START;
			return true;
		default:
			rVirtualKey = VirtualKey::Undefined;
			rOldStyleKey = 0;
			return false;
		};
	}
	...
	vks::VirtualKey convertRawKey(const uint32_t rawKeyCode)
	{
		using namespace vks;
...
void VulkanExampleBase::handleKeyEvent(const vks::VirtualKey virtualKey, const bool isPressed, const uint32_t rawKeyCode)
{
	using namespace vks;
				...
				switch (action) {
					...
					case AMOTION_EVENT_ACTION_DOWN: {
						...
						if (eventTime - vulkanExample->lastTapTime <= vks::android::DOUBLE_TAP_TIMEOUT) {
							...
							if ((x * x + y * y) < deadZone) {
								vulkanExample->handleKeyEvent(VirtualKey::TouchDoubleTap, true, TOUCH_DOUBLE_TAP);
								vulkanExample->handleKeyEvent(VirtualKey::TouchDoubleTap, false, TOUCH_DOUBLE_TAP);
								vulkanExample->touchDown = false;
							}
						}
						else {
...
void VulkanExampleBase::onKeyEvent(const vks::VirtualKey virtualKey, const bool isPressed, const uint32_t rawKeyCode)
{
	// Can be overriden in derived class
}
