...
public:
	virtual int32_t			init(CVulkanFramework* pFramework)
	{
		...
		m_pFramework->zoom = -5.5;
		m_pFramework->zoomSpeed = 2.5f;
		m_pFramework->rotationSpeed = 0.5f;
		m_pFramework->rotation = { -25.5, -123.75, 0.0 };
		m_pFramework->title = "Vulkan Example - Mesh rendering";
		return 0;
	}
	...
	virtual ~VulkanExample()
	{
		...
		vkDestroyBuffer(m_pFramework->device, mesh.vertices.buf, nullptr);
		vkFreeMemory(m_pFramework->device, mesh.vertices.mem, nullptr);
		vkDestroyBuffer(m_pFramework->device, mesh.indices.buf, nullptr);
		vkFreeMemory(m_pFramework->device, mesh.indices.mem, nullptr);
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < m_pFramework->drawCmdBuffers.size(); ++i) {
			...
			vkCmdBindVertexBuffers(m_pFramework->drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1, &mesh.vertices.buf, offsets);
			// Bind mesh index buffer
			vkCmdBindIndexBuffer(m_pFramework->drawCmdBuffers[i], mesh.indices.buf, 0, VK_INDEX_TYPE_UINT32);
			// Render mesh vertex buffer using it's indices
			vkCmdDrawIndexed(m_pFramework->drawCmdBuffers[i], mesh.indices.count, 1, 0, 0, 0);
...
#endif
		meshLoader->LoadMesh(m_pFramework->getAssetPath() + "models/voyager/voyager.obj");
