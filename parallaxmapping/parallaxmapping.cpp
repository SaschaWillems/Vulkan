...
void reBuildCommandBuffers()
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			VkViewport viewport = vkTools::initializers::viewport(
				(splitScreen) ? (float)ScreenRect.Width / 2.0f : (float)ScreenRect.Width,
			...
			if (splitScreen)
			{
				viewport.x = (float)ScreenRect.Width / 2.0f;
				vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);
	...
	void updateUniformBuffers()
	{
		...
		ubos.vertexShader.projection = glm::perspective(deg_to_rad(45.0f), (float)(ScreenRect.Width* ((splitScreen) ? 0.5f : 1.0f)) / (float)ScreenRect.Height, 0.001f, 256.0f);
