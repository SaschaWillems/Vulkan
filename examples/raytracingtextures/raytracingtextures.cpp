	...
	void createDescriptorSets()
	{
		...
		for (auto i = 0; i < maxConcurrentFrames; i++) {
			...
			std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
				...
				vks::initializers::writeDescriptorSet(descriptorSets[i], VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 3, &texture.descriptor),
			};
	...
	*/
	...
	void buildCommandBuffer()
	{
		...
		*/
		...
		vkCmdPushConstants(cmdBuffer, pipelineLayout, VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_ANY_HIT_BIT_KHR, 0, sizeof(uint64_t) * 2, &bufferReferences);
